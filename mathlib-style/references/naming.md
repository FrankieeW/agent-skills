...
Contrast this with the rule for documentation,
...
Some widely used predicates don't follow this rule. Those are the predicates that are analogous to an atom already suffixed by the naming convention:
...
for the "normal subgroup" typeclass, but `Normal` is also fine.
